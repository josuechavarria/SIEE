...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	if (!ISSET($_POST['listaIdsRoles'])){
		...
		if (!($listaIdsRoles = $_POST['listaIdsRoles'])){
			$response['refresh_error'] = 'Debes seleccionar almenos un rol para realizar esta acción.';
		}else{
			$query = 'UPDATE siee_rol SET activo = CASE activo WHEN 1 THEN 0 ELSE 1 END WHERE id IN(';
			foreach($listaIdsRoles as $id){
				$query .= $id . ',';
			}
			$query = substr($query, 0, strlen($query) - 1) . ');';
			
			$stmt_cambiar_estado_rol = $conn->prepare($query);
			
			$conn->exec('BEGIN TRANSACTION');
			
			try {				
				if (!$stmt_cambiar_estado_rol->execute()){
				...
				$conn->exec('ROLLBACK TRANSACTION');
				$stmt_cambiar_estado_rol->closeCursor();
