...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	if (!ISSET($_POST['id']) || !ISSET($_POST['titulo']) || !ISSET($_POST['descripcion'])){
		...
		if (!($titulo = trim($_POST['titulo']))){
			$errores['TituloFuenteDato_mod'] = 'Escriba aqui el titulo de la fuente de datos.';
		}else{
			if (strlen($titulo) > 128){
				$errores['TituloFuenteDato_mod'] = 'El titulo de la fuente de datos excede el tama&ntilde;o maximo de 128 caracteres.';
			}else{
				if (!preg_match($patron_titulo, $titulo)){
					$errores['TituloFuenteDato_mod'] = $patron_titulo_err;
				}
			}
		}
		
		if (!($descripcion = trim($_POST['descripcion']))){
			$errores['DescripcionFuenteDato_mod'] = 'Escriba aqui la descripcion de la fuente de datos.';
		}else{
			if (strlen($descripcion) > 256){
				$errores['DescripcionFuenteDato_mod'] = 'La descripcion de la fuente de datos excede el tama&ntilde;o maximo de 256 caracteres.';
			}else{
				if (!preg_match($patron_descripcion, $descripcion)){
					$errores['DescripcionFuenteDato_mod'] = $patron_descripcion_err;
				}
			}
		}
		
		if (sizeof($errores) == 0){					
			$idFuenteDato = $_POST['id'];
			$usuario_modificador_id = $_SESSION['usuario']['id'];
			
			
			$stmt_modificar_fuente_dato = $conn->prepare("UPDATE siee_fuente_dato SET
															titulo = ?,
															descripcion = ?,
			...
			try {				
				if (!$stmt_modificar_fuente_dato->execute(array(utf8_decode($titulo), utf8_decode($descripcion), $usuario_modificador_id, $idFuenteDato))){
				...
				$conn->exec('ROLLBACK TRANSACTION');
				$stmt_modificar_fuente_dato->closeCursor();
