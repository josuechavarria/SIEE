...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	if (!ISSET($_POST['listaIdsSubsitio'])){
		...
		if (!($listaIdsSubsitio = $_POST['listaIdsSubsitio'])){
			$response['refresh_error'] = 'Debes seleccionar almenos un subsitio para realizar esta acción.';
		}else{
			$query = 'UPDATE siee_subsitio SET activo = CASE activo WHEN 1 THEN 0 ELSE 1 END WHERE id IN(';
			foreach($listaIdsSubsitio as $id){
				$query .= $id . ',';
			}
			$query = substr($query, 0, strlen($query) - 1) . ');';
			
			$stmt_cambiar_estado_subsitio = $conn->prepare($query);
			
			$conn->exec('BEGIN TRANSACTION');
			
			try {				
				if (!$stmt_cambiar_estado_subsitio->execute()){
				...
				$conn->exec('ROLLBACK TRANSACTION');
				$stmt_cambiar_estado_subsitio->closeCursor();
