...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	if (!ISSET($_POST['listaIdsTiposMatricula'])){
		...
		if (!($listaIdsTiposMatricula = $_POST['listaIdsTiposMatricula'])){
			$response['refresh_error'] = 'Debes seleccionar almenos un tipo de matricula para realizar esta acción.';
		}else{
			$query = 'UPDATE siee_tipo_matricula SET activo = CASE activo WHEN 1 THEN 0 ELSE 1 END WHERE id IN(';
			foreach($listaIdsTiposMatricula as $id){
				$query .= $id . ',';
			}
			$query = substr($query, 0, strlen($query) - 1) . ');';
			
			$stmt_cambiar_estado_tipo_matricula = $conn->prepare($query);
			
			$conn->exec('BEGIN TRANSACTION');
			
			try {				
				if (!$stmt_cambiar_estado_tipo_matricula->execute()){
				...
				$conn->exec('ROLLBACK TRANSACTION');
				$stmt_cambiar_estado_tipo_matricula->closeCursor();
