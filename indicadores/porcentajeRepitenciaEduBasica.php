...
if (ISSET($_POST['anio']) && ISSET($_POST['departamento']) && ISSET($_POST['municipio']) && ISSET($_POST['centro']) && ISSET($_POST['desagregacion'])) {
    ...
    if (is_numeric($anio) && ctype_digit($anio) && is_numeric($departamento) && ctype_digit($departamento) && is_numeric($municipio) && ctype_digit($municipio) && is_numeric($centro) && ctype_digit($centro) && is_numeric($desagregacion) && ctype_digit($desagregacion)) {
        $indicador = selectIndicadorPorId(54);
        ...
        $repitente_PromedioNacional = $DatosParametrizados_PromedioNacional['reprobados'];
        ...
        $repitente_anioBase = $DatosParametrizados_anioBase['reprobados'];
        ...
        if($desagregacion === '0'){
            ...
            $repitente = $DatosParametrizados['reprobados'];
            ...
            //calculo del indicador segun los detalles solicitados en la formula
            $DatosParametrizados = $DatosQuery['DatosParametrizados'];        
            $repitente_A = $DatosParametrizados['reprobados_A'];
            $matricula_A = $DatosParametrizados['matricula_A'];
            $repitente_B = $DatosParametrizados['reprobados_B'];
                    ...
                    $(document).ready(function() {
                        chart = new Highcharts.Chart({
                            ...
                            series: [{
                                    ...
                                    data: [
                                        ['Aprobados',   <?php echo ($matricula - $repitente) ?>],
                                        ['Reprobados',   <?php echo $repitente ?>]
                                    ]
                                }]
                            <?php }else{ ?>
                                 series: [{
                                    ...
                                    name: '<?php echo utf8_encode($indicador['titulo']) ?>',
                                    data: [
                                        ['Aprobados <?php echo utf8_encode($etiqueta_A) ?>',   <?php echo ($matricula_A - $repitente_A) ?>],
                                        ['Reprobados <?php echo utf8_encode($etiqueta_A) ?>',   <?php echo $repitente_A ?>]
                                    ],
                                    center:['30%','50%'],
                                    size:200
                                },{
                                    ...
                                    name: '<?php echo utf8_encode($indicador['titulo']) ?>',
                                    data: [
                                        ['Aprobados <?php echo utf8_encode($etiqueta_B) ?>',   <?php echo ($matricula_B - $repitente_B) ?>],
                                        ['Reprobados <?php echo utf8_encode($etiqueta_B) ?>',   <?php echo $repitente_B ?>]
                                    ],
                                    center:['65%','50%'],
                                    size:200
                                }]
