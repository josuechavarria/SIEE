...
if (ISSET($_POST['anio']) && ISSET($_POST['departamento']) && ISSET($_POST['municipio']) && ISSET($_POST['centro']) && ISSET($_POST['desagregacion'])) {
    ...
    if (is_numeric($anio) && ctype_digit($anio) && is_numeric($departamento) && ctype_digit($departamento) && is_numeric($municipio) && ctype_digit($municipio) && is_numeric($centro) && ctype_digit($centro) && is_numeric($desagregacion) && ctype_digit($desagregacion)) {
        $indicador = selectIndicadorPorId(60);
        ...
        $desertor_PromedioNacional = $DatosParametrizados_PromedioNacional['desertores'];
        $matricula_PromedioNacional = $DatosParametrizados_PromedioNacional['matricula'];
      
        $valorIndicador_PromedioNacional = (  $desertor_PromedioNacional / $matricula_PromedioNacional) * 100;

        //calculo del datos para el año 
        $DatosParametrizados_anioBase = $DatosQuery['DatosAnioBase'];
        $desertor_anioBase = $DatosParametrizados_anioBase['desertores'];
        $matricula_anioBase = $DatosParametrizados_anioBase['matricula'];
      
        $valorIndicador_anioBase = (   $desertor_anioBase / $matricula_anioBase) * 100;       
        ...
        if($desagregacion === '0'){
            ...
            $desertor = $DatosParametrizados['desertores'];
            $matricula = $DatosParametrizados['matricula'];
           
            $valorIndicador = (  $desertor / $matricula) * 100;
            
        }else{
            //calculo para el indicador con desagregaciones.
            //calculo del indicador segun los detalles solicitados en la formula
            $DatosParametrizados = $DatosQuery['DatosParametrizados'];        
            $desertor_A = $DatosParametrizados['desertores_A'];
            $matricula_A = $DatosParametrizados['matricula_A'];
            $desertor_B = $DatosParametrizados['desertores_B'];
            $matricula_B = $DatosParametrizados['matricula_B'];
            $valorIndicador_A = ( $desertor_A / $matricula_A ) * 100;
            $valorIndicador_B = (  $desertor_B / $matricula_B) * 100;
                            ...
                            if ($desagregacion === '0') {
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td>  
                                            <?php echo number_format($desertor, 0) ?>
                                        </td> 
                                    </tr>
                                    <tr>
                                        <th>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td>  
                                            <?php echo number_format($desertor_A, 0) ?>
                                        </td> 
                                        <td>  
                                            <?php echo number_format($desertor_B, 0) ?>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>
                    ...
                    $(document).ready(function() {
                        chart = new Highcharts.Chart({
                            ...
                            series: [{
                                    ...
                                    data: [
                                        ['No desertores',   <?php echo ($matricula - $desertor) ?>],
                                        ['Desertores',   <?php echo $desertor ?>]
                                    ]
                                }]
                            <?php }else{ ?>
                                 series: [{
                                    ...
                                    name: '<?php echo utf8_encode($indicador['titulo']) ?>',
                                    data: [
                                        ['No desertores <?php echo utf8_encode($etiqueta_A) ?>',   <?php echo ($matricula_A - $desertor_A) ?>],
                                        ['Desertores <?php echo utf8_encode($etiqueta_A) ?>',   <?php echo $desertor_A ?>]
                                    ],
                                    center:['30%','50%'],
                                    size:200
                                },{
                                    ...
                                    name: '<?php echo utf8_encode($indicador['titulo']) ?>',
                                    data: [
                                        ['No desertores <?php echo utf8_encode($etiqueta_B) ?>',   <?php echo ($matricula_B - $desertor_B) ?>],
                                        ['Desertores <?php echo utf8_encode($etiqueta_B) ?>',   <?php echo $desertor_B ?>]
                                    ],
                                    center:['65%','50%'],
                                    size:200
                                }]
